...
func (info *NetworkInfo) GetHNSNetworkConfig() *hcsshim.HNSNetwork {
	...
	return &hcsshim.HNSNetwork{
		...
		Subnets:       subnets,
		DNSServerList: strings.Join(info.DNS.Nameservers, ","),
		DNSSuffix:     info.DNS.DomainSuffix,
		SourceMac:     "",
...
func GetNetworkInfo(hnsNetwork *hcsshim.HNSNetwork) *NetworkInfo {
	...
	return &NetworkInfo{
		...
		Subnets:       subnets,
		DNS: DNSInfo{
			DomainSuffix: hnsNetwork.DNSSuffix,
			Nameservers:  strings.Split(hnsNetwork.DNSServerList, ","),
		},
